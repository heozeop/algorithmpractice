...
int getGlobalMax(int x, int y, int count, int curMax) {
  ...
  for(int i = 0; i < 4; ++i) {
    ...
    localMax = max(localMax, getGlobalMax(nx,ny,count + 1, curMax + paperMap[nx][ny]));
    paperVisitMap[nx][ny] = false;
  }

  if(x-1 >= 0 && y-1 >= 0 && x+1 < n) { //ㅓ
      localMax = max(localMax, (paperMap[x-1][y] + paperMap[x][y-1] + paperMap[x][y] + paperMap[x+1][y]));
  }
  if(x-1 >= 0 && y+1 < m && x+1 < n) { //ㅏ
      localMax = max(localMax, (paperMap[x-1][y] + paperMap[x][y+1] + paperMap[x][y] + paperMap[x+1][y]));
  }
  if(y-1 >= 0 && y+1 < m && x+1 < n) { //ㅗ
      localMax = max(localMax, (paperMap[x][y] + paperMap[x+1][y] + paperMap[x+1][y-1] + paperMap[x+1][y+1]));
  }
  if(y-1 >= 0 && y+1 < m && x+1 < n) { //ㅜ
      localMax = max(localMax, (paperMap[x][y-1] + paperMap[x][y] + paperMap[x][y+1] + paperMap[x+1][y]));
  }
