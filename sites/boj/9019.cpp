...
void init() { fill(&dp[0], &dp[VALUE_LIMIT - 1] + 1, VALUE_LIMIT); }

int calcD(int n) { return n * 2 % VALUE_LIMIT; }
...
int calcL(int n) { return (n % 1000) * 10 + n / 1000; }

int calcR(int n) { return (n % 10) * 1000 + n / 10; }
...
string gogogo() {
  ...
  while (!q.empty()) {
    ...
    int afterD = calcD(curVal), afterS = calcS(curVal), afterL = calcL(curVal),
        afterR = calcR(curVal);
