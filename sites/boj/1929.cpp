/*
  ...
  입력: 첫째 줄에 자연수 M과 N이 빈 칸을 사이에 두고 주어진다. (1 ≤ M ≤ N ≤
  1,000,000) M이상 N이하의 소수가 하나 이상 있는 입력만 주어진다. 출력: 한 줄에
  하나씩, 증가하는 순서대로 소수를 출력한다.
*/
/*
  소수 출력하기 -> 소수 판단하기 ->
    루트n: 주어인 수 a를 판별하기 위해 필요한 연산은 루트 a, 이걸 1~100만까지 ->
  O(n루트n) 아리스토테네스의 체: m ~ n까지 늘어놓고, 소수와 그 배수를 전부 삭제
  <- root n은 써야 할 듯 하다. 밀러-라빈 소수판별법: 페르마의 소정리 이용해 소수
  판별, 이건 블로그로 써야할 듯 소수의 정의 -> 자기자신과 자기 자신의
  반수(modulo연산의 덧셈)로만 나누어 떨어지는 셈
*/
