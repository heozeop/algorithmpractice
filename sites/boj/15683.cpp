  ...
  case 2:
    return (*function)(index, location, direction) // on direction
           + (*function)(index, location,
                         (direction + 2) % 4); // opposite direction
  case 3:
    return (*function)(index, location, direction) // on direction
           + (*function)(index, location,
                         (direction + 1) % 4); // righthand side direction
  case 4:
    return (*function)(index, location, direction) // ondirection
           + (*function)(index, location,
                         (direction + 1) % 4) // righthand side direction
           + (*function)(index, location,
                         (direction + 3) % 4); // lefthand side direction
  case 5:
...
int main(void) {
  ...
  cout << row * col - numOfUncensoredArea -
              getNumOfCensoredArea(FIRSTCAMERAID, DEFAULTVALUE);
}