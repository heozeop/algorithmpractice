...
int countFallAmount(int row, int col) {
  int fallCount = 0;
  while(row > 0 && caveMap[row][col] == MINERAL) {row++;}
...
pbi getFallInfo(int row, int col) {
  ...
  while(!q.empty()) {
    ...
    if (counting[col]) {
      answer.second = min(answer.second, countFallAmount(curP.first, curP.second));
      counting[col] = false;
    }

    for (int i = 0; i < 4; i++) {
      int nextR = curP.first + direction[i][0], nextC = curP.second + direction[i][1];
