 ...
 *        세 번째 줄부터 n줄에 걸쳐 행성계의 중점과 반지름 (cx, cy, r)이
 * 주어진다. 출력: 각 테스트 케이스에 대해 어린 왕자가 거쳐야 할 최소의 행성계
 * 진입/이탈 횟수를 출력한다.
 *
 *
 *  생각:
 *    출발점이 포함된 원 + 도착점이 포함된 원 - 둘다 포함된 원 (행성계 교차,
 * 접촉 없음) 출발점과 도착점이 원에 포함되는지 판별해야함 (점 - 원점) < 반지름
 * (경계에 걽치지 않는다고 가정)
 */
